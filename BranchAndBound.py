...
def getAllNodes( edgeL ):
    nodeL = []
    for edge in edgeL:
        if edge[0] not in nodeL:
            nodeL.append(edge[0])
        if edge[1] not in nodeL:
            nodeL.append(edge[1])
    return nodeL
...
def hasCycle(edgeL):
    ...
    def visit( node ):
        ...
        visited.append(node)
        path.append(node)
        neighborL = getNeighborStops( edgeL, node )
    ...
    return any(visit(node) for node in nodeL)
...
def main():
    ...
    #print(pokeList)
    print(hasCycle(getAllEdges(pokeList)))
